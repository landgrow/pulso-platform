-- =============================================================================
-- Migration 0018 — Diretório de clientes exclui a org interna da Land Grow
-- =============================================================================
-- A org interna (organizations.is_internal = true, seed em 0012) não tem
-- registro em `clientes` — hoje ela aparece em /admin/clientes como uma linha
-- vazia (sem dono, sem time, sem contrato) porque admin_client_directory()
-- nunca filtrava por is_internal. Agora que a org interna virou um módulo de
-- verdade dentro do admin (Atividades, Mapa Mental, Painel, BIN, MIN), ela
-- não deve mais aparecer misturada na lista de clientes.
-- =============================================================================
...
COMMENT ON FUNCTION public.admin_client_directory(uuid) IS
  'Lista completa (ou uma linha, se p_org_id for passado) de clientes para a área admin, incluindo encerrados mas excluindo a org interna da Land Grow (is_internal). Restrita a platform_admin/consultant.';
